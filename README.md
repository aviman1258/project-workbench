...
- **AI drafting** (`src/lib/ai-complete.ts`, `src/lib/ai/*`): "Draft from repository" reads the linked repo/PR through the GitHub API (owner-allowlisted, secrets redacted, budgeted), drafts the description and why, reconstructs UI pages as mockup images from the HTML/CSS/JS, and assembles a how-it-works PDF. Calls Anthropic directly from the browser with the owner's API key; OpenAI keys work through the bundled Cloudflare Worker proxy (`proxy/openai-worker.js`).
